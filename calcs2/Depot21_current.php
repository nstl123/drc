...
class Depot21 {
	...
	public function getDemandData($countryIDs, $scenarioID, $typeID, $pwrID, $isRegion, $showAtDeviceLevel, $perHH) { 				
		...
		for ($u = 2006; $u < 2022; $u++) {
			...
			if (($isRegion > 0) && ($perHH)) {
				$stmntGeo	   = "rg.namen as countryName, nc.".$useCluster." as regionID";
				...
				$stmntGeo	   = "nc.namen as countryName, nc.".$useCluster." as regionID";
				$sumStmntInner = $sumStmntInner.", ".$this->perHHmultiplier." * sum( dm.Y".$u." / sdt.Y".$u." ) AS Y".$u;
			} else if ($isRegion > 0) {
				$stmntGeo	   = "rg.namen as countryName, nc.".$useCluster." as regionID";
		...
		$corePart = ", dm.countryID AS countryID,". $stmntGeo.$sumStmntInner.$sumStmntPop."
				...
				WHERE dm.scenarioID IN (10001) AND nc.".($isRegion > 0 ? $useCluster : "id")." IN  ".$countryList." 
				AND sdt.indicatorID = 101".
				($typeID > 0 ? " AND batTypeID = ".$typeID : "" ).($pwrID > 0 ? " AND pwrTypeID = ".$pwrID : "" )."
			GROUP BY scenarioID, indicatorID";
		
		$corePartDevices    = "SELECT dm.scenarioID, 301 AS indicatorID, dm.deviceID, 
								devNam.namen AS deviceName, devNam.categoryID as categoryID, devCat.namen as categoryName
								".$corePart."countryID, deviceID";
		$corePartCategories = "SELECT dm.scenarioID, 301 AS indicatorID, devNam.categoryID as deviceID, devNam.namen AS deviceName,
								devCat.id as categoryID, devCat.namen as categoryName
							  ".$corePart.",countryID devNam.categoryID";
		$corePartAll	    = "SELECT dm.scenarioID, 301 AS indicatorID, null as deviceID, null AS deviceName,
								null as categoryID, null as categoryName".$corePart;
		
		if ( ($isRegion > 0) && ($perHH > 0) ) { // no outer wrapper SQL otherwise
			//scenarioID, indicatorID, countryID, regionID
			$samePartDevices    = " * ".$sumStmntOuter." 
				FROM (".$corePartDevices.") AS a GROUP BY scenarioID, regionID";		
			$samePartCategories = " * ".$sumStmntOuter."
				FROM (".$corePartCategories.") AS a GROUP BY scenarioID, regionID";						
		} else if ($perHH > 0) {		
			$samePartDevices = " * FROM (".$corePartDevices.") AS a";
			$samePartCategories = " * ".$sumStmntOuter." FROM (".$corePartCategories.") AS a";
		} else if ($isRegion > 0) {
			//scenarioID, indicatorID, countryID, regionID
			$samePartDevices = " * ".$sumStmntOuter."
				FROM (".$corePartDevices.") AS a  GROUP BY scenarioID, regionID";		
			...
			$samePartDevices    = " * FROM (".$corePartDevices.") AS a";
			$samePartCategories = " * FROM (".$corePartCategories.") AS a";
		};		
		...
		if ($hasOtherCountries > 0) {	// flag showing there are countries besides World aggregate						
			if    ( ($showAtDeviceLevel == 0) && ($isRegion > 0) ) $mainStmnt = "SELECT * FROM (".$corePartAll.", nc.region) AS a";
			else if ($showAtDeviceLevel == 0) $mainStmnt = "SELECT * FROM (".$corePartAll.", countryID) AS a";
			
			if    ( ($showAtDeviceLevel > 0) && ($isRegion > 0) ) $mainStmnt = "SELECT ".$samePartDevices.", deviceID";
			else if ($showAtDeviceLevel > 0) $mainStmnt = "SELECT ".$samePartDevices;
			
			if ($showAtDeviceLevel  > 0) {
				$mainStmnt = $mainStmnt."\r\nUNION\r\n";		
				if ($isRegion > 0) 	$mainStmnt = $mainStmnt."SELECT ".$samePartCategories.", deviceID";
				else 				$mainStmnt = $mainStmnt."SELECT ".$samePartCategories;
			};
		};
