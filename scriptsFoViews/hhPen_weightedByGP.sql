CREATE ALGORITHM=UNDEFINED DEFINER=`cFullUserPW`@`%` SQL SECURITY DEFINER VIEW `Consulting`.`DC_scenarioDataIndisWeightedBySz` AS 

select `wg`.`scenarioID` AS `scenarioID`,`wg`.`countryID` AS `countryID`, 213 AS `indicatorID`, -- adjusted HH pen rate
`wg`.`deviceID` AS `deviceID`,0 AS `typeID`,
...
select `wg`.`scenarioID` AS `scenarioID`,`wg`.`countryID` AS `countryID`, 214 AS `indicatorID`, -- adjusted Market Size 
`wg`.`deviceID` AS `deviceID`,0 AS `typeID`,
...
select 
`wg`.`scenarioID` AS `scenarioID`,`wg`.`countryID` AS `countryID`, 215 AS `indicatorID`, -- adjusted devBase
`wg`.`deviceID` AS `deviceID`,0 AS `typeID`,
